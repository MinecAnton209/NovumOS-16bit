...
| [Эмулятор — Обзор](emulator/overview.md) | Архитектура эмулятора, сборка, тестирование |
| [Ассемблерные обертки](wrappers/overview.md) | Высокоуровневый Zig API для генерации кода ISA |
...
| Команды | MOV, ADD, SUB, INC, DEC, CMP, TEST, AND, OR, XOR, NOT, NEG, SHL, SHR, XCHG, ADC, SBB, PUSH, POP, JMP, JZ, JNZ, JC, JNC, JS, JNS, CALL, RET, INT, IRET, IN, OUT, NOP, HLT |
| Эмулятор | Циклически точный, 38 CPU + 23 теста оберток проходят |
...
- [x] Циклически точный эмулятор CPU (38 CPU + 23 теста оберток проходят)
- [x] Генератор прошивки и система сборки
- [x] Высокоуровневые ассемблерные обертки (asm.zig)
