...
The ALU instruction supports 16 sub-operations encoded into a single opcode (4-bit field):

| Value | Mnemonic | Description |
|-------|----------|-------------|
| 0x0   | ADD      | `dst = dst + src` |
| 0x1   | ADC      | `dst = dst + src + C` |
| 0x2   | SUB      | `dst = dst - src` |
| 0x3   | SBB      | `dst = dst - src - C` |
| 0x4   | CMP      | Compare (flags only, no result) |
| 0x5   | TEST     | Bitwise AND (flags only, no result) |
| 0x6   | AND      | `dst = dst AND src` |
| 0x7   | OR       | `dst = dst OR src` |
| 0x8   | XOR      | `dst = dst XOR src` |
| 0x9   | SHL      | `dst = dst << src` |
| 0xA   | SHR      | `dst = dst >> src` |
| 0xB   | INC      | `dst = dst + 1` |
| 0xC   | DEC      | `dst = dst - 1` |
| 0xD   | NOT      | `dst = NOT dst` (bitwise complement) |
| 0xE   | NEG      | `dst = 0 - dst` (two's complement negate) |
| 0xF   | XCHG     | Swap `dst` and `src` |
